...
<div id="lx-root" style="height:100vh;width:100%;overflow:hidden;background:var(--bg);color:var(--text);font-family:var(--font);">
  <template v-if="vm.showLogin">
    <div style="height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;background:radial-gradient(120% 90% at 50% 0%, var(--surface-base) 0%, var(--bg) 60%);">
      <div style="width:400px;max-width:100%;display:flex;flex-direction:column;gap:22px;animation:lx-pop .4s ease;">
        <div style="display:flex;flex-direction:column;align-items:center;gap:14px;">
          <div style="width:52px;height:52px;border-radius:15px;background:var(--accent);color:var(--accent-contrast);display:flex;align-items:center;justify-content:center;font:600 26px/1 var(--display);box-shadow:var(--shadow-md);">灵</div>
        ...
        <div style="background:var(--panel);border:1px solid var(--line);border-radius:18px;padding:24px;box-shadow:var(--shadow);display:flex;flex-direction:column;gap:14px;">
          ...
          <button @click="vm.submitAuth" :disabled="vm.authBusy" style="margin-top:4px;height:44px;border:0;border-radius:11px;background:var(--accent);color:var(--accent-contrast);font:600 14.5px/1 var(--font);cursor:pointer;box-shadow:var(--shadow);display:flex;align-items:center;justify-content:center;gap:7px;">{{ vm.authBusy ? '请稍候…' : (vm.isRegister ? '注册并进入' : '登录') }} <i class="ph ph-arrow-right"></i></button>
  ...
  <template v-if="vm.authed">
    <div :style="vm.shellStyle">
      <nav :style="vm.railStyle">
        <div style="width:38px;height:38px;border-radius:11px;background:var(--accent);color:var(--accent-contrast);display:flex;align-items:center;justify-content:center;font:600 19px/1 var(--display);box-shadow:var(--shadow);margin-bottom:8px;">灵</div>
        ...
        <div @click="vm.goSettings" :title="vm.sName" style="width:34px;height:34px;border-radius:50%;background:var(--surface-active);color:var(--text-secondary);display:flex;align-items:center;justify-content:center;font:600 13px/1 var(--font);margin-top:6px;cursor:pointer;">{{ vm.meBig }}</div>
      </nav>
      <template v-if="vm.notifOpen">
        <div @click="vm.closeNotif" style="position:fixed;inset:0;z-index:40;"></div>
        <div style="position:fixed;left:74px;bottom:16px;width:340px;max-width:80vw;background:var(--panel);border:1px solid var(--line2);border-radius:16px;box-shadow:var(--shadow-lg);z-index:41;overflow:hidden;animation:lx-pop .2s ease;">
      ...
      <template v-if="vm.searchOpen">
        <div @click="vm.closeSearch" style="position:fixed;inset:0;background:var(--overlay-scrim);z-index:50;display:flex;align-items:flex-start;justify-content:center;padding-top:12vh;">
          <div @click="vm.stop" style="width:560px;max-width:90vw;background:var(--panel);border:1px solid var(--line2);border-radius:16px;box-shadow:var(--shadow-lg);overflow:hidden;animation:lx-pop .18s ease;">
      ...
      <template v-if="vm.shortcutsOpen">
        <div @click="vm.closeShortcuts" style="position:fixed;inset:0;background:var(--overlay-scrim);z-index:52;display:flex;align-items:center;justify-content:center;padding:24px;">
          <div @click="vm.stop" style="width:430px;max-width:92vw;background:var(--panel);border:1px solid var(--line2);border-radius:16px;box-shadow:var(--shadow-lg);overflow:hidden;animation:lx-pop .18s ease;">
      ...
      <template v-if="vm.isMobile">
        <div style="flex:0 0 auto;height:52px;display:flex;align-items:center;gap:10px;padding:0 12px;border-bottom:1px solid var(--line);background:var(--panel);">
          ...
          <div style="width:28px;height:28px;border-radius:8px;background:var(--accent);color:var(--accent-contrast);display:flex;align-items:center;justify-content:center;font:600 14px/1 var(--display);">灵</div>
      ...
      <div :style="vm.paneWrapStyle" @drop="vm.onPaneDrop" @dragover="vm.onPaneOver">
        <template v-if="vm.paneDragActive">
          <div style="position:absolute;inset:0;z-index:40;pointer-events:none;display:flex;gap:0;">
            <div style="flex:1;margin:10px;border:2px dashed var(--accent);border-radius:16px;background:var(--accent-bg);opacity:.6;display:flex;align-items:center;justify-content:center;"><span style="font:600 13px/1 var(--font);color:var(--accent-ink);display:inline-flex;align-items:center;gap:6px;"><i class="ph ph-arrow-line-left"></i>放到左侧</span></div>
          ...
          <template v-if="vm.showAdminContent">
            ...
            <div style="flex:1;overflow:auto;padding:26px 24px;">
              ...
              <div v-if="vm.hasAdminUser" style="max-width:760px;margin:0 auto;display:flex;flex-direction:column;gap:18px;">
                <div style="display:flex;align-items:center;gap:14px;"><span style="width:46px;height:46px;border-radius:13px;background:var(--surface-active);color:var(--text-secondary);display:flex;align-items:center;justify-content:center;font:600 18px/1 var(--display);">{{ vm.auName.slice(-1) }}</span><div style="flex:1;"><div style="font:600 17px/1.2 var(--display);color:var(--text);">{{ vm.auName }} <span style="font:600 11px/1 var(--font);color:var(--accent-ink);background:var(--accent-bg);border-radius:20px;padding:3px 8px;margin-left:6px;">{{ vm.auRole }}</span></div><div style="font:500 12.5px/1.3 var(--font);color:var(--text3);margin-top:2px;">{{ vm.auEmail }}</div></div><span style="padding:5px 11px;border-radius:20px;background:var(--accent-bg);color:var(--accent-ink);font:600 11.5px/1 var(--font);">{{ vm.auStatus }}</span></div>
                ...
                <div style="background:var(--panel);border:1px solid var(--line);border-radius:14px;overflow:hidden;box-shadow:var(--shadow);">
                  ...
                  <template v-for="(r, __i26) in vm.adminLog" :key="__i26"><div style="display:grid;grid-template-columns:1fr 84px 1fr 74px;gap:0;border-bottom:1px solid var(--line);align-items:center;"><div style="padding:11px 14px;font:500 12.5px/1.4 var(--font);color:var(--text2);">{{ r.raw }}</div><div style="padding:11px 8px;"><span :style="`display:inline-flex;align-items:center;gap:4px;font:600 11px/1 var(--font);color:${r.kc};`"><span :style="`width:6px;height:6px;border-radius:50%;background:${r.kc};`"></span>{{ r.kind }}</span></div><div style="padding:11px 8px;font:500 12.5px/1.4 var(--font);color:var(--text);">{{ r.result }}</div><div style="padding:11px 8px;font:500 11px/1.3 var(--font);color:var(--text3);"><span class="lx-mono">{{ r.gen }}</span></div></div></template>
...
class Component {
  ...
  _colorPool = ['var(--cat-1)','var(--cat-2)','var(--cat-3)','var(--cat-4)','var(--cat-5)'];
  _memberColor(name){ if(!name) return 'var(--cat-fallback)'; let h=0; for(let i=0;i<name.length;i++) h=(h*31+name.charCodeAt(i))|0; return this._colorPool[Math.abs(h)%this._colorPool.length]; }
  ...
  TOK = {
    // App light/dark map onto the Attio design tokens (src/styles/tokens.css).
    // Both maps are identical — the actual light↔dark switch is driven by the
    // data-theme attribute set in applyTheme(), which tokens.css responds to.
    // --accent is intentionally omitted so it inherits from :root (setting it to
    // var(--accent) here would be a cyclic self-reference and invalidate it).
  ...
  async loadState(){
    try{
      ...
      this.setState({ projects:(st.projects||[]).map((p,idx)=>({ id:p.id, name:p.name, desc:p.description||'', status:p.status||'进行中', color:['var(--cat-1)','var(--cat-2)','var(--cat-4)','var(--cat-3)'][idx%4] })) });
  ...
  undoEntity(msg){
    ...
    p.then(()=>{
      this.setState(s=>({
        tasks: kind==='task'? s.tasks.filter(x=>x.id!==refId): s.tasks,
        ideas: kind==='idea'? s.ideas.filter(x=>x.id!==refId): s.ideas,
        nonTodos: kind==='nono'? s.nonTodos.filter(x=>x.id!==refId): s.nonTodos,
        feed: s.feed.filter(f=>f.refId!==refId),
        messages: s.messages.map(x=>x.id===msg.id?{id:x.id,role:'sys',text:'已撤销：'+String(title).slice(0,30)}:x),
        detailId: s.detailId===refId?null:s.detailId,
      }));
      this.flashToast('已撤销');
    }).catch(e=>this.flashToast('撤销失败：'+e.message));
  }
  ...
  commitPlan(msg){
    ...
    if(!items.length){ this.flashToast('该计划没有可执行的任务'); return; }
    api.commitPlan(items).then(r=>{
      const map=new Map((r.updated||[]).map(t=>[t.id,this._mapTask(t)]));
      this.setState(s=>({ tasks:s.tasks.map(x=>map.has(x.id)?{...x,...map.get(x.id)}:x), messages:s.messages.map(x=>x.id===msg.id?{...x,committed:true}:x) }));
      this.flashToast('已写入执行计划 · 在「今日」视图查看');
    }).catch(e=>this.flashToast('操作失败：'+e.message));
  }
  // 历史回链：用户消息 → 它生成的实体
  openEntity(type,id){
    if(type==='task'){ if(this.state.tasks.some(t=>t.id===id)){ this.openTask(id); } else this.flashToast('该任务已被删除或移出'); return; }
    if(type==='todo_idea'){ if(this.state.ideas.some(i=>i.id===id)){ this.setState({view:'clarify',selIdeaId:id,mobilePane:'main'}); } else this.flashToast('该想法已被处理'); return; }
    if(this.state.nonTodos.some(n=>n.id===id)){ this.setState({view:'nontodo',selNonId:id,mobilePane:'main'}); } else this.flashToast('该记录已被处理');
  }
  leaveCollabTask(id){
    if(!window.confirm('退出协作后，这个任务将从你的列表中移除。确定退出吗？')) return;
    api.leaveTask(id).then(()=>{
      this.setState(s=>({tasks:s.tasks.filter(t=>t.id!==id),detailId:null,feed:s.feed.filter(f=>f.refId!==id)}));
      this.flashToast('已退出协作');
    }).catch(e=>this.flashToast('操作失败：'+e.message));
  }
  respondInviteUi(n,mode){
  ...
  submitNewProject(){
    ...
    if(!name){ this.flashToast('请输入项目名称'); return; }
    api.createProject(name,'').then(p=>{
      this.setState(s=>({projects:[...s.projects,{id:p.id,name:p.name,desc:p.description||'',status:'进行中',color:this._colorPool[s.projects.length%this._colorPool.length]}],newProjOpen:false,newProjName:'',selProjectId:p.id}));
      this.flashToast('项目已创建 · 聊天里提到项目名会自动归属');
    }).catch(e=>this.flashToast('创建失败：'+e.message));
  }
  saveOwnAi(){
    api.updateOwnAiConfig(this._aiCfg()).then(()=>{ this.setState(s=>({aiSource:'own',settings:{...s.settings,apiKey:''}})); this.flashToast('已保存个人 AI 配置（仅对你生效）'); }).catch(e=>this.flashToast('保存失败：'+e.message));
  }
  clearOwnAi(){
    api.clearOwnAiConfig().then(()=>this.loadState().then(()=>this.flashToast('已恢复使用团队配置'))).catch(e=>this.flashToast('操作失败：'+e.message));
  }
  retry(msgId,text){
  ...
  async send() {
    ...
    try{
      ...
      const onDelta=(d)=>{
        if(!d) return;
        if(!streamId){
          streamId='m'+(++this._seq);
          this.setState(s=>({thinking:false,messages:[...s.messages,{id:streamId,role:'ai',kind:'text',text:d,streaming:true}]}), ()=>this.scrollMsgs());
        } else {
          this.setState(s=>({messages:s.messages.map(m=>m.id===streamId?{...m,text:m.text+d}:m)}), ()=>this.scrollMsgs());
        }
      };
      let res;
      try{
      ...
      for(const p of (res.performed||[])){
        ...
        else if(p.type==='convert_idea'){ ideas=ideas.filter(x=>x.id!==p.ideaId); feedArr=feedArr.filter(f=>f.refId!==p.ideaId); }
      ...
      if(res.plan&&res.plan.length){
        newMsgs.push({id:'m'+(++this._seq),role:'ai',kind:'plan',planTitle:'接下来 · 建议计划',planSub:'基于当前可见 todo',planNote:'未使用非 todo 内容制定计划',plan:res.plan.map((p,i)=>({n:i+1,t:p.task.title,d:(p.minutes||30)+' min',id:p.task.id,m:p.minutes||30}))});
      }
  ...
  inviteMember(taskId,u,force){
    api.inviteCollab(taskId,u.id,force).then(r=>{ this.flashToast(r.reused?(u.name+' 已在协作名单里'):('已邀请 '+u.name+'（待接受）')); this.openTask(taskId); })
      .catch(e=>{
        if(e&&e.status===409&&window.confirm(e.message||'个人任务，确认邀请？')){ this.inviteMember(taskId,u,true); return; }
        this.flashToast('邀请失败：'+e.message);
      });
  }
  inviteAll(taskId){
    const existing=new Set((this.state.taskCollabs[taskId]||[]).filter(c=>['pending','accepted','following'].includes(c.status)).map(c=>c.userId));
    const me=this.state.settings.name;
    const targets=(this.state.team||[]).filter(u=>u.name!==me&&!existing.has(u.id));
    if(!targets.length){ this.flashToast('没有可邀请的成员了'); return; }
    Promise.allSettled(targets.map(u=>api.inviteCollab(taskId,u.id))).then(rs=>{
      const ok=rs.filter(r=>r.status==='fulfilled').length;
      this.flashToast('已向 '+ok+' 位成员发出邀请'); this.openTask(taskId);
    });
  }
  deleteRule(id){ api.deleteAutoRule(id).then(()=>{ this.setState(s=>({autoRules:s.autoRules.filter(r=>r.id!==id)})); this.flashToast('已删除自动规则'); }).catch(e=>this.flashToast('删除失败：'+e.message)); }
  ...
  paneDragStart(e){ if(this.state.isMobile) return; this._paneDragging=true; this.setState({paneDragActive:true}); try{ if(e&&e.dataTransfer){ e.dataTransfer.effectAllowed='move'; e.dataTransfer.setData('text/plain','pane'); } }catch(_){} }
  paneDragEnd(){ this._paneDragging=false; this.setState({paneDragActive:false}); }
  onPaneOver(e){ if(this._paneDragging&&e&&e.preventDefault) e.preventDefault(); }
  onPaneDrop(e){ if(!this._paneDragging) return; if(e&&e.preventDefault)e.preventDefault(); this._paneDragging=false; const x=e.clientX||0; const target=x<=(window.innerWidth/2); this.setState({paneDragActive:false}); this.swapPanes(target); }
  // FLIP 动画：先记录旧位置 → 换 order → 反向位移再过渡到 0，形成"滑到另一侧"的补位效果
  swapPanes(target){
    if(this.state.isMobile) return;
    if(target===this.state.paneSwap){ return; }
    const els=[document.getElementById('lx-mid'),document.getElementById('lx-main')].filter(Boolean);
    const first=els.map(e=>e.getBoundingClientRect().left);
    this.setState({paneSwap:target}, ()=>{
      els.forEach((e,i)=>{ const dx=first[i]-e.getBoundingClientRect().left; if(!dx)return; e.style.transition='none'; e.style.transform=`translateX(${dx}px)`; });
      requestAnimationFrame(()=>requestAnimationFrame(()=>{ els.forEach(e=>{ e.style.transition='transform .28s cubic-bezier(.4,0,.2,1)'; e.style.transform=''; }); }));
      try{localStorage.setItem('lx_pane_swap', target?'1':'0');}catch(_){}
      this.flashToast(target?'对话框已移到左侧':'对话框已移到右侧');
    });
  }
  ...
  fmtTask(t){
    ...
    return {
      id:t.id, title:t.title, project:t.project, due:t.due, statusLabel: t.collabFrom?statusLabel+' · 来自 '+t.collabFrom:statusLabel, collabFrom:t.collabFrom||null,
  ...
  renderVals() {
    ...
    // messages
    const messages=st.messages.map(m=>({...m,isSys:m.role==='sys',isUser:m.role==='user',isAgentText:m.role==='ai'&&m.kind==='text',isTask:m.role==='ai'&&m.kind==='task',isIdea:m.role==='ai'&&m.kind==='idea',isNono:m.role==='ai'&&m.kind==='nono',isPlan:m.role==='ai'&&m.kind==='plan',isError:m.role==='ai'&&m.kind==='error',retry:()=>this.retry(m.id,m.retryText),undo:()=>this.undoEntity(m),commitPlan:()=>this.commitPlan(m),openRef:()=>this.openEntity(m.refType,m.refId),hasRefs:!!(m.refs&&m.refs.length),open:()=>{if(m.kind==='task')this.openTask(m.refId);else if(m.kind==='idea')this.setState({view:'clarify'});}}));
    // database
    ...
    const dbViewName={all:'全部任务',today:'今日',open:'未完成',done:'已完成',collab:'协作任务'}[st.dbView];
    ...
    const counts2={collab:visTasks.filter(t=>t.collabFrom).length};
    const dbDefs=[['all','全部任务','ph-stack'],['today','今日','ph-sun-horizon'],['open','未完成','ph-circle-dashed'],['done','已完成','ph-check-circle'],['collab','协作任务','ph-users']];
    counts.collab=counts2.collab;
    ...
    // 负责人从真实团队目录选（含自己 + 已出现过的负责人兜底）
    const memberNames=[...new Set([...(st.team||[]).map(u=>u.name),st.settings.name||'我',...st.tasks.map(t=>t.assignee).filter(Boolean)])].filter(Boolean).slice(0,8);
    ...
    // 协作人区：状态芯片 + owner 邀请入口
    const collabMeta={pending:['待接受','var(--idea)','var(--idea-bg)'],accepted:['协作中','var(--accent-ink)','var(--accent-bg)'],following:['关注中','var(--text2)','var(--mid)']};
    const dCollabsRaw=dt?(st.taskCollabs[dt.id]||[]).filter(c=>collabMeta[c.status]):[];
    const dCollabs=dCollabsRaw.map(c=>({name:c.userName,label:collabMeta[c.status][0],fg:collabMeta[c.status][1],bg:collabMeta[c.status][2],dotc:collabMeta[c.status][1]}));
    const dIsOwner=dt?(((st.taskAccess[dt.id])||'owner')==='owner'&&!dt.collabFrom):false;
    const inviteCandidates=(st.team||[]).filter(u=>u.name!==st.settings.name&&!dCollabsRaw.some(c=>c.userId===u.id)).map(u=>({name:u.name,invite:()=>this.inviteMember(st.detailId,u)}));
    ...
    const setSw=(on)=>({track:'width:38px;height:22px;border-radius:12px;cursor:pointer;flex:0 0 auto;position:relative;background:'+(on?'var(--accent)':'var(--line2)')+';',knob:'position:absolute;top:3px;left:'+(on?'19px':'3px')+';width:16px;height:16px;border-radius:50%;background:var(--gray-0);box-shadow:var(--shadow-sm);'});
    ...
    return {
      ...
      paneDragStart:(e)=>this.paneDragStart(e), paneDragEnd:()=>this.paneDragEnd(), onPaneDrop:(e)=>this.onPaneDrop(e), onPaneOver:(e)=>this.onPaneOver(e), paneDragActive:st.paneDragActive, paneSwap:st.paneSwap,
      ...
      dCollabFrom:dt?(dt.collabFrom||''):'', leaveCollab:()=>this.leaveCollabTask(st.detailId),
      dCollabs, dIsOwner, invitePickerOpen:st.invitePickerOpen, toggleInvitePicker:()=>this.setState(s=>({invitePickerOpen:!s.invitePickerOpen})), inviteCandidates, inviteAll:()=>this.inviteAll(st.detailId),
      ...
      aiOwnActive:st.aiSource==='own', ownAiOpen:st.ownAiOpen, toggleOwnAi:()=>this.setState(s=>({ownAiOpen:!s.ownAiOpen})), saveOwnAi:()=>this.saveOwnAi(), clearOwnAi:()=>this.clearOwnAi(),
